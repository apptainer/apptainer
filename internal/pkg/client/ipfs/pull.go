...
func pull(ctx context.Context, imgCache *cache.Handle, directTo, pullFrom string) (imagePath string, err error) {
	...
	cid := strings.Split(strings.Replace(pullFrom, "ipfs://", "", 1), "/")[0]

	if cid[0] != 'b' { // multibase: base32
		return "", fmt.Errorf("unknown encoding for cid: %c", cid[0])
	}
	...
	if len(data) != 4+256/8 {
		return "", fmt.Errorf("wrong length for data: %d", len(data))
	}
	if data[0] != 1 { // cid: v1
		return "", fmt.Errorf("unknown version for cid: %d", data[0])
	}
	if data[1] != 0x70 { // multicodec: dag-pb
		return "", fmt.Errorf("unknown codec for cid: 0x%x", data[1])
	}
	if data[2] != 0x12 { // multihash code: sha2
		return "", fmt.Errorf("unknown code for hash: 0x%x", data[2])
	}
	if data[3] != 0x20 { // multihash length: 256 bits
		return "", fmt.Errorf("unknown length for hash: 0x%x", data[3])
	}
	hash := hex.EncodeToString(data[4:])
...
// Pull will pull an ipfs image to the cache or direct to a temporary file if cache is disabled
func Pull(ctx context.Context, imgCache *cache.Handle, pullFrom string, tmpDir string) (imagePath string, err error) {
	...
	if imgCache.IsDisabled() {
		...
		directTo = file.Name()
		sylog.Infof("Downloading ipfs image to tmp cache: %s", directTo)
	}

	return pull(ctx, imgCache, directTo, pullFrom)
}

// PullToFile will pull an ipfs image to the specified location, through the cache, or directly if cache is disabled
