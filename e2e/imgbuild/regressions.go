...
func (c *imgBuildTests) issue476(t *testing.T) {
	tmpDir, cleanup := e2e.MakeTempDir(t, c.env.TestDir, "issue-476-", "")
	defer cleanup(t)

	// running under the mode 1(https://apptainer.org/docs/user/main/fakeroot.html)
	c.env.RunApptainer(
		t,
		e2e.WithProfile(e2e.UserProfile),
		e2e.WithCommand("build"),
		e2e.WithArgs("--force", "--fakeroot", fmt.Sprintf("%s/openssh-mode1.sif", tmpDir), "testdata/regressions/issue_476.def"),
		e2e.ExpectExit(0),
	)
