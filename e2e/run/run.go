...
// testRunCDI checks that passing --device and --cdi-dirs work as expected
func (c ctx) testRunCDI(t *testing.T) {
	...
	// Test CDI with device path specifications.
	// Use --contain because usually apptainer imports everything in /dev.
	cdiJSON := `{
		"cdiVersion": "0.5.0",
	...
	for _, tt := range devTests {
		t.Run(tt.name, func(t *testing.T) {
			cmdArgs := []string{"--contain"}
			if tt.withCdi {
				cmdArgs = append(cmdArgs, "--device", "apptainer.org/device=testDevice", "--cdi-dirs", cdiDir)
			}
			cmdArgs = append(cmdArgs, imageRef, "ls", "-ld", tt.devPath)
